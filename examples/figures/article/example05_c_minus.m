...
for i = 1:(2 * N)
    ...
    if mod(i, 2) == 0
        text(mean(v(1, :)), mean(v(2, :)), mean(v(3, :)) - 0.32, sprintf('Connected component %d', i / 2))
    end
end

[LEGH, OBJH, OUTH, OUTM] = legend; % reading handles
[~, objh] = legend([OUTH; plot_path], OUTM{:}, 'Connected components', 'Location', 'northeast'); % append new plot
