...
$y=R\tilde{y}\Leftrightarrow \tilde{y}=R^Ty$, $R^TR=E$.

$\tilde{y}_i=R_{ji}y_j$.

{\bf unchecked}

$\tilde{A}_{ilk}=R_{ji}A_{jlk},\,\tilde{b}_{il}=R_{ji}b_{jl}$

$(\tilde{c}\cdot \tilde{b})_l=\tilde{c}_i \tilde{b}_{il}=R_{ki}c_k R_{ji}b_{jl}=0\Leftrightarrow c^TRR^T\left|\left|
\begin{array}{c}
b_1^T\\
...\\
b_m^T
\end{array}
\right|\right|=0\Leftrightarrow || b_1 ... b_m ||RR^Tc=0 \Leftrightarrow ||b_1 ... b_m||c=0$

Therefore, there is no point in changing $y$ space linearly.
\end{enumerate}
\subsection*{6. $z(c)=?$}
